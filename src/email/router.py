...
def update_smtp_setting_settings(
    ...
    try:
        updated_smtp = update_smtp_setting(smtp_id, server, port, username, password, sender)
        if not updated_smtp:
...
def delete_smtp_setting_settings(smtp_id: int = Path(..., description="ID SMTP настройки для для удаления")):
    deleted_count = delete_smtp_setting(smtp_id)
    if deleted_count == 0:
        raise HTTPException(status_code=404, detail="SMTP настройка не найдена")
